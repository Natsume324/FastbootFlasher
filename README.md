...
华为：解析与提取.APP/update.bin文件中的分区镜像，合并两段或三段 sparse super 分区，选择分区刷写，刷入成功与失败统计，fastboot模式读取信息，fastboot模式跳转升级模式  
fastboot：随附版本同时兼容标准 Android fastboot，以及使用华为 WinUSB GUID / FF-FF-00 接口的 HarmonyOS NEXT 设备（HarmonyOS NEXT 设备需正确绑定 WinUSB 驱动）。  
